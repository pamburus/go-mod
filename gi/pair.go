...
func PairFold[V1, V2, R any, F ~func(V1, V2) R](pairs iter.Seq2[V1, V2], fold F) iter.Seq[R] {
	return PairFoldFunc(fold)(pairs)
}

// PairFoldFunc returns a function that transforms an iterator sequence over pairs to an iterator sequence over values.
// It uses the provided fold function to transform pairs to values.
func PairFoldFunc[V1, V2, R any, F ~func(V1, V2) R](fold F) func(iter.Seq2[V1, V2]) iter.Seq[R] {
...
func PairUnfold[V, R1, R2 any, F ~func(V) (R1, R2)](values iter.Seq[V], unfold F) iter.Seq2[R1, R2] {
	return PairUnfoldFunc(unfold)(values)
}

// PairUnfoldFunc returns a function that transforms an iterator sequence over values to an iterator sequence over pairs.
// It uses the provided unfold function to transform values to pairs.
func PairUnfoldFunc[V, R1, R2 any, F ~func(V) (R1, R2)](unfold F) func(iter.Seq[V]) iter.Seq2[R1, R2] {
